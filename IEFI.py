...
try:
    aplicacion.iconbitmap(icon_path)
except Exception as e:
    print(f"Error al cargar el ícono: {e}")

# Estilos ajustados
estilo_label = {"font": ("Trebuchet MS", 12, "bold"), "bg": "#edf2f7", "fg": "#2d3436"}
estilo_checkbutton = {"bg": "#edf2f7", "activebackground": "#ffffff", "fg": "#000000", "font": ("Trebuchet MS", 10, "bold")}
estilo_boton = {"bg": "#0f2775", "fg": "white", "font": ("Trebuchet MS", 11, "bold"), "relief": "raised"}
...
estilo_TMenu.map("TMenubutton",
                 background=[("active", "#d0455a"), ("pressed", "#c0394f")],
...
marca_path = os.path.join(script_dir, 'marca.png')
try:
    marca = tk.PhotoImage(file=marca_path)
    marca = marca.subsample(2, 2)
    label_marca = tk.Label(frame_cliente, image=marca, bg="#e35165")
    label_marca.pack(side="left", padx=10)
except Exception as e:
    print(f"Error al cargar la imagen de la marca: {e}")
...
# Asignación de precio al ítem correspondiente
